...
def faild_Allposts():
    ...
    assert response.status_code == 500
def faild_DeletePost():
    test_url = addr + '/post/delete'
    PARAMS = {'post_id':"1"} 
    response = s.get(test_url,params = PARAMS)
    assert response.status_code == 500
def faild_postUpdate():
...
faild_storeReadWithPosts()
faild_allStore()
faild_storeDelete()



def faild_commentCreate():
    ...
    response = s.post(test_url, headers=headers,data = PARAMS)
    assert response.status_code == 500
def faild_commentRead():
...
faild_findByMobileId()


def faild_mobileName():
...
def faild_recommendMobile():
    ...
    response = s.post(test_url, headers=headers,data = json.dumps(PARAMS))
    assert response.status_code == 500
faild_recommendMobile() 


def faild_recommendStore():
    ...
    response = s.get(test_url,headers=headers,params = PARAMS)
    
    assert response.status_code == 500
faild_recommendStore()
    
#-------------------------------------------------- 
def faild_videoDetection():
    test_url = addr + '/croped_video'
    path = os.path.join( os.getcwd(), 'project/public/videos')+"/20500807_203246.mp4"
